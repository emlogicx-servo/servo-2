    ...
    "steps": {
      ...
      "options": {
        ...
        "sql": {
          ...
          "query": "SELECT *\nFROM servo_order_items\nINNER JOIN servo_orders ON (servo_orders.order_id = servo_order_items.servo_orders_order_id) INNER JOIN servo_user ON (servo_user.user_id = servo_orders.servo_user_user_id)\nWHERE servo_user.user_id = :P1 /* {{$_GET.user_id}} */ AND servo_orders.servo_shift_shift_id = :P2 /* {{$_GET.current_shift}} */ AND servo_orders.order_status = :P3 /* {{$_GET.order_status}} */",
          "params": [
            {
              "operator": "equal",
              "type": "expression",
