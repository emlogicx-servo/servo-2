...
$app->define(<<<'JSON'
{
  ...
  "exec": {
    "steps": [
      ...
      {
        "name": "shift_adjustments",
        "module": "dbupdater",
        "action": "custom",
        "options": {
          "connection": "servodb",
          "sql": {
            "query": "select sum(order_total_adjustment) AS TotalAdjustment from servo_orders where servo_shift_shift_id like :P1 and servo_service_service_id like :P2",
            "params": [
              {
                "name": ":P1",
                "value": "{{$_GET.shift_id}}",
