...
$app->define(<<<'JSON'
{
  ...
  "exec": {
    "steps": [
      ...
      {
        "name": "shift_sales_products_accessories",
        "module": "dbupdater",
        "action": "custom",
        "options": {
          "connection": "servodb",
          "sql": {
            "query": "SELECT *, sum(order_item_quantity) as Volume, sum(order_item_price * order_item_quantity) as Total\nFROM servo_order_items\nLEFT JOIN servo_products ON servo_products.product_id = servo_order_items.servo_products_product_id \n\nLEFT JOIN servo_product_categories ON servo_product_categories.product_categories_id = servo_products.servo_product_category_product_category_id \n\nLEFT JOIN servo_orders ON servo_orders.order_id = servo_order_items.servo_orders_order_id\nWHERE servo_orders.servo_shift_shift_id = :P1 /* {{$_GET.shift_id}} */ AND servo_orders.servo_service_service_id AND order_item_group_type = 'Accessory' like :P2 /* {{$_GET.service_id}} */\n\ngroup by product_id\n",
            "params": [
              {
                "name": ":P1",
                "value": "{{$_GET.shift_id}}",
      ...
      {
        "name": "shift_sales_products_ingredients",
        "module": "dbupdater",
        "action": "custom",
        "options": {
          "connection": "servodb",
          "sql": {
            "query": "SELECT *, sum(order_item_quantity) as Volume, sum(order_item_price * order_item_quantity) as Total\nFROM servo_order_items\nLEFT JOIN servo_products ON servo_products.product_id = servo_order_items.servo_products_product_id \n\nLEFT JOIN servo_product_categories ON servo_product_categories.product_categories_id = servo_products.servo_product_category_product_category_id \n\nLEFT JOIN servo_orders ON servo_orders.order_id = servo_order_items.servo_orders_order_id\nWHERE servo_orders.servo_shift_shift_id = :P1 /* {{$_GET.shift_id}} */ AND servo_orders.servo_service_service_id AND order_item_group_type = 'Ingredient' like :P2 /* {{$_GET.service_id}} */\n\ngroup by product_id\n",
            "params": [
              {
                "name": ":P1",
                "value": "{{$_GET.shift_id}}",
