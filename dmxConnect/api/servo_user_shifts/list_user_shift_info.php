      ...
      },
      {
        ...
        "options": {
          ...
          "sql": {
            ...
            "params": [
              {
                ...
                "type": "expression",
                "name": ":P1",
                "value": "{{query_list_user_shift[0].servo_service_sales_point}}",
                "test": ""
              }
            ],
            "table": {
            ...
            "query": "select `servo_customers`.`customer_id`, `servo_customers`.`customer_first_name`, `servo_customers`.`customer_last_name`, `servo_sales_point`.`sales_point_id` from `servo_customers` left join `servo_sales_point` on `servo_sales_point`.`sales_point_customer_id` = `servo_customers`.`customer_id` where `servo_sales_point`.`sales_point_id` = ?",
            "wheres": {
              "condition": "AND",
              "rules": [
                {
