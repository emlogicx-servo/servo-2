...
$app->define(<<<'JSON'
{
  ...
  "exec": {
    "steps": [
      ...
      {
        "name": "transactions_shift_pos_payment_methods",
        "module": "dbupdater",
        "action": "custom",
        "options": {
          "sql": {
            "query": "SELECT sum(transaction_amount) as Total, payment_method_name \nFROM servo_customer_cash_transaction \nLEFT JOIN servo_orders ON servo_orders.order_id = servo_customer_cash_transaction.transaction_order LEFT JOIN servo_payment_methods ON servo_payment_methods.payment_method_id = servo_customer_cash_transaction.transaction_payment_method\nWHERE servo_orders.servo_shift_shift_id = :P1 /* {{$_GET.shift_id}} */ AND servo_customer_cash_transaction.customer_id = :P2 /* {{$_GET.customer_id}} */ AND servo_customer_cash_transaction.transaction_type='Settlement'\ngroup by payment_method_name",
            "params": [
              {
                "name": ":P1",
                "value": "{{$_GET.shift_id}}",
